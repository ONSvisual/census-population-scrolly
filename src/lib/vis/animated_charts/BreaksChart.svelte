<script>
	...
	export let breaks;
	export let colors = breaks.length==5?["#e9e9e9","#FED4A6","#FDA13C","#ED7320"]:["#3E558A","#958BBA","#CAC6DE","#e9e9e9","#FED4A6","#FDA13C","#ED7320"];
...
{#if highlow && Array.isArray(highlow)}
<div class="highlow">
	{#each highlow.slice(0,2) as hl, i}
	{#if i == 0}
	<span style:color="{colors[0]}"><Icon rotation={180} position="left"/>{hl}</span>
	{:else}
	<span style:color="{colors[breaks.length - 2]}">{hl}<Icon position="right"/></span>
	{/if}
	{/each}
</div>
{/if}
